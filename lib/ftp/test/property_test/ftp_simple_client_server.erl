...
prop_ftp_targeted() ->
  ?FORALL_TARGETED(
     Cmds, proper_statem:targeted_commands(?MODULE),
     aggregate(command_names(Cmds),
               begin
                 {_H, S, Result} = run_commands(?MODULE, Cmds),
                 [cmnd_stop_server(X) || X <- S#state.servers],
                 [ftp:stop_service(X) || {ok,X} <- S#state.clients],
                 ?MAXIMIZE(length(S#state.store)),
                 ?WHENFAIL(io:format("Result = ~p~n", [Result]),
                           Result == ok)
               end)).

%% This function is for calls from common_test test cases:
prop_ftp(Config) ->
  numtests(?v(numtests, Config),
           prop_ftp(filename:join([?v(property_dir,Config), ?MODULE_STRING++"_data"]),
                    ?v(priv_dir,Config), Config)).


prop_ftp(DataDir, PrivDir, Config) ->
  S0 = #state{data_dir = DataDir,
              priv_dir = PrivDir},
  ?FORALL(Cmds,  more_commands(10,commands(?MODULE,S0)),
          aggregate(command_names(Cmds),
                    begin {_H,S,Result} = run_commands(?MODULE,Cmds),
                          ...
                          [cmnd_stop_server(X) || X <- S#state.servers],
                          [ftp:stop_service(X) || {ok,X} <- S#state.clients],
                          ct_property_test:present_result(?MODULE, Cmds, {_H, S, Result}, Config),
                          Result==ok
                    end)).
...
initial_state() ->
  ?fmt("Initial_state()~n",[]),
  {ok, PWD} = file:get_cwd(),
  #state{data_dir = filename:join([PWD,?MODULE_STRING++"_data"]),
         priv_dir = filename:join([PWD,?MODULE_STRING,"_files"])}.
...
command(#state{servers=Ss=[_|_],
  ...
  frequency([
             ...
             {20, {call,?MODULE,cmnd_delete,[oneof(Cs),oneof(Store)]}}
            ]);
...
list_commands(#state{servers=Ss, clients=Cs, priv_dir=PrivDir, data_dir=DataDir, store=Store}) ->
  [{call, ?MODULE, cmnd_init, [PrivDir]},
   {call, ?MODULE, cmnd_start_server, [PrivDir, DataDir]},
   {call, ?MODULE, cmnd_start_client, [oneof(Ss) || Ss =/= []]},
   {call, ?MODULE, cmnd_stop_client, [oneof(Cs) || Cs =/= []]},
   {call, ?MODULE, cmnd_put, [oneof(Cs) || Cs =/= []] ++ [file_path(), file_contents()]},
   {call, ?MODULE, cmnd_get, [oneof(Cs) || Cs =/= []] ++ [oneof(Store) || Store =/= []]},
   {call, ?MODULE, cmnd_delete, [oneof(Cs) || Cs =/= []] ++ [oneof(Store) || Store =/= []]}].
...
precondition(#state{initialized=IsInit, clients=Cs}, {call, _, cmnd_put, [C,_,_]}) ->
  IsInit == true andalso lists:member(C,Cs);
precondition(_, {call, _, cmnd_put, _}) -> false;

precondition(#state{initialized=IsInit, clients=Cs, store=Store},
             {call, _, cmnd_get, [C,X]}) -> IsInit == true andalso lists:member(C,Cs) andalso lists:member(X,Store);
precondition(_, {call, _, cmnd_get, _}) -> false;


precondition(#state{initialized=IsInit, clients=Cs, store=Store},
             {call, _, cmnd_delete, [C,X]}) -> IsInit == true andalso lists:member(C,Cs) andalso lists:member(X,Store);
precondition(_, {call, _, cmnd_delete, _}) -> false;

precondition(#state{initialized=IsInit, servers=Ss}, {call, _, cmnd_start_client, _}) ->
  IsInit == true andalso Ss =/= [];

precondition(#state{initialized=IsInit, clients=Cs}, {call, _, cmnd_stop_client, _}) ->
  IsInit == true andalso Cs =/= [];

precondition(#state{initialized=IsInit, servers=Ss}, {call, _, cmnd_start_server, _}) -> IsInit == true andalso Ss == []  ;
